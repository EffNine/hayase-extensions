export default new class SeaDex {
  ...
  async single({anilistId, titles, episodeCount, fetch: fetchFn}, options) {
    if (!navigator.onLine) return [];
    if (!anilistId) throw new Error("No anilistId provided");
    if (!titles?.length) throw new Error("No titles provided");
    const res = await fetchFn(`${this.url}?page=1&perPage=1&filter=alID%3D%22${anilistId}%22&skipTotal=1&expand=trs`);
