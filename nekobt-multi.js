...
export default new class NekoBT {
  ...
  async single({tvdbId: tvdbId, tvdbEId: tvdbEId, tmdbId: tmdbId, episode: episode, fetch: fetch, resolution: resolution, exclusions: exclusions}, options) {
    ...
    ep?.id && searchParams.append("episode_ids", ep.id.toString()), options?.language && (searchParams.append("fansub_lang", options.language.toString()), 
    searchParams.append("sub_lang", options.language.toString()));
