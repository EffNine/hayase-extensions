function _parseItems(string) {
  ...
  for (const itemMatch of string.matchAll(itemRegex)) {
    const itemString = itemMatch[1], item = {};
    const enclosureMatch = itemString.match(/<enclosure url="([^"]+)"[^>]*>/);
    ...
    nzbnameMatch && (item.nzbname = nzbnameMatch[1]);
    items.push(item);
  }
  return items;
}

function _findByName(string, name) {
  const items = _parseItems(string);
